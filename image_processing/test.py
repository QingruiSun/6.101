...
def test_inverted_2():
    originated_img = {"height": 1, "width": 4, "pixels": [23, 80, 149, 193]}
    expected_img = {"height": 1, "width": 4, "pixels": [232, 175, 106, 162]}
    result = lab.inverted(originated_img)
    compare_images(result, expected_img)


def test_correlate_1():
    centered_img = lab.load_greyscale_image(
        os.path.join(TEST_DIRECTORY, "test_images", "centered_pixel.png")
    )
    kernel = [0, 0, 0, 0, 1, 0, 0, 0, 0]
    correlate_img = lab.correlate(centered_img, kernel, "extend")
    compare_images(correlate_img, centered_img)


def test_correlate_2():
    originated_img = {"height": 3, "width": 3, "pixels": [1, 2, 3, 2, 3, 2, 3, 1, 4]}
    kernel = [1, 1, 1, 1, 1, 1, 1, 1, 1]
    correlate_img = lab.correlate(originated_img, kernel, "extend")
    expected_img = {
        "height": 3,
        "width": 3,
        "pixels": [15, 19, 23, 18, 21, 24, 21, 23, 25],
    }
    compare_images(correlate_img, expected_img)
...
def test_blurred_black_image():
    ...
    black_img = {"height": 6, "width": 5, "pixels": [0] * 30}
    result_img = lab.blurred(black_img, kernsize)
    expected_img = {"height": 6, "width": 5, "pixels": [0] * 30}
